...
#                     xlow = 0  # Default value or logic
#                     xhigh = 0  # Default value or logic
#                     all_avg_true_sizes = [0]* len(all_avg_true_sizes)
#                     all_avg_pred_sizes = [0]* len(all_avg_true_sizes)
...
#                     pred_density_100_perimage_list = [0]*len(image_name_list_100_density)
#                 # xlow = int(min(true_density_100_perimage_list) - 0.1*(max(true_density_100_perimage_list)-min(true_density_100_perimage_list)))
#                 # xhigh = int(max(true_density_100_perimage_list) + 0.1*(max(true_density_100_perimage_list)-min(true_density_100_perimage_list)))
#                 #xlow = 0.0
#                 #xhigh = 10.0
...
#                 if true_avg_shape_bd_perimage_list and pred_avg_shape_bd_perimage_list:
#                     xlow = int(min(true_avg_shape_bd_perimage_list) - 0.1*(max(true_avg_shape_bd_perimage_list)-min(true_avg_shape_bd_perimage_list)))
#                     xhigh = int(max(true_avg_shape_bd_perimage_list) + 0.1*(max(true_avg_shape_bd_perimage_list)-min(true_avg_shape_bd_perimage_list)))
#                 else:
...
#                 print(f"Length of true_avg_shape_bd_perimage_list: {len(true_avg_shape_bd_perimage_list)}")
#                 print(f"Length of pred_avg_shape_bd_perimage_list: {len(pred_avg_shape_bd_perimage_list)}")
#                 print(f"Length of image_name_list_bd_size: {len(image_name_list_bd_shape)}")
...
def analyze_checkpoints(cfg, defect_metadata, input_yaml, iou_score_threshold_test, test_dataset_path,
                        anno_dict_list_val, file_note, only_last_checkpoint=True,
                        true_and_pred_matching_method='iou_bbox', true_and_pred_matching_thresholds=None):
    """
    Analyzes model checkpoints for defect detection.
    """
    def read_yaml_thresholds(input_yaml, default_thresholds):
        if isinstance(input_yaml, dict):
            return input_yaml.get('true_and_pred_matching_thresholds', default_thresholds)
        try:
            with open(input_yaml, 'r') as f:
                yaml_data = yaml.safe_load(f)
            return yaml_data.get('true_and_pred_matching_thresholds', default_thresholds)
        except Exception as e:
            raise RuntimeError(f"Error reading YAML: {e}")

    def save_per_image_stats(data_dict, output_dir, prefix):
        with open(os.path.join(output_dir, f'{prefix}_Stats.json'), 'w') as f:
            json.dump(data_dict, f)

    def calculate_stats(data_dict, output_dir, prefix):
        best, worst = {}, {}
        metrics = ["avg size error", "avg density error", "overall F1", "defect find F1"]
        for metric in metrics:
            values = {img: data.get(metric, float('inf') if 'F1' in metric else 0) for img, data in data_dict.items()}
            best[metric] = min(values, key=values.get)
            worst[metric] = max(values, key=values.get)
        save_per_image_stats(best, output_dir, f'{prefix}_BestImages')
        save_per_image_stats(worst, output_dir, f'{prefix}_WorstImages')

    def plot_and_save_parity_plots(true_vals, pred_vals, x_label, y_label, output_path, color, label, aspect_ratio=0.5):
        fig, ax = get_fig_ax(aspect_ratio)
        ax.scatter(true_vals, pred_vals, color=color, edgecolors='black', s=100, zorder=2, alpha=0.7, label=label)
        ax.set_xlabel(x_label)
        ax.set_ylabel(y_label)
        ax.legend(loc='lower right')
        xlow, xhigh = min(true_vals) * 0.9, max(true_vals) * 1.1
        ax.set_xlim(xlow, xhigh)
        ax.set_ylim(xlow, xhigh)
        ax.plot([xlow, xhigh], [xlow, xhigh], color='black', lw=1.5, linestyle='--')
        r2, mae, rmse = r2_score(true_vals, pred_vals), mean_absolute_error(true_vals, pred_vals), np.sqrt(mean_squared_error(true_vals, pred_vals))
        plot_stats(fig, {'R2': r2, 'MAE': mae, 'RMSE': rmse}, x_align=0.15, y_align=0.90)
        fig.savefig(output_path, dpi=250, bbox_inches='tight')
        
    def match_defects(true_data, pred_data, method='iou_bbox', threshold=None):
        """
        Matches true and predicted defects based on the specified method.
        
        Args:
            true_data (tuple): Contains true classes, segmentations, and bounding boxes.
            pred_data (tuple): Contains predicted classes, segmentations, and bounding boxes.
            method (str): Matching method, either 'iou_bbox' or 'pixelnorm_mask'.
            threshold (float): Threshold value for matching (IoU or pixel norm).

        Returns:
            num_found (int): Number of matched defects.
            stats (dict): A dictionary of statistics per image.
        """
        true_classes, true_segmentations, true_boxes = true_data
        pred_classes, pred_segmentations, pred_boxes = pred_data

        num_found = 0
        stats = {
            "true_sizes": [], "pred_sizes": [],
            "true_densities": [], "pred_densities": [],
            "true_shapes": [], "pred_shapes": []
        }

        if method == 'iou_bbox':
            for true_box, true_class in zip(true_boxes, true_classes):
                for pred_box, pred_class in zip(pred_boxes, pred_classes):
                    iou = calculate_iou(true_box, pred_box)
                    if iou >= threshold:
                        num_found += 1
                        stats["true_sizes"].append(true_box["size"])
                        stats["pred_sizes"].append(pred_box["size"])
                        stats["true_shapes"].append(true_box["shape"])
                        stats["pred_shapes"].append(pred_box["shape"])
        elif method == 'pixelnorm_mask':
            for true_seg, true_class in zip(true_segmentations, true_classes):
                for pred_seg, pred_class in zip(pred_segmentations, pred_classes):
                    norm = calculate_pixel_norm(true_seg, pred_seg)
                    if norm <= threshold:
                        num_found += 1
                        stats["true_sizes"].append(true_seg["size"])
                        stats["pred_sizes"].append(pred_seg["size"])
                        stats["true_shapes"].append(true_seg["shape"])
                        stats["pred_shapes"].append(pred_seg["shape"])
        else:
            raise ValueError("Invalid method. Must be 'iou_bbox' or 'pixelnorm_mask'.")

        # Add density calculations (placeholder logic, adapt as needed)
        stats["true_densities"] = [len(true_classes) / (true_data[2].size or 1)]
        stats["pred_densities"] = [len(pred_classes) / (pred_data[2].size or 1)]

        return num_found, stats
    ...
    def calculate_pixel_norm(seg1, seg2):
        """
        Calculates the pixel-wise Euclidean norm between two segmentations.
        """
        return np.linalg.norm(np.array(seg1) - np.array(seg2))


    # Load thresholds from YAML if not provided
    default_thresholds = [0.7]
    true_and_pred_matching_thresholds = true_and_pred_matching_thresholds or read_yaml_thresholds(input_yaml, default_thresholds)

    # Log thresholds
    print(f"Using thresholds: {true_and_pred_matching_thresholds}")

    validation_image_filenames, model_checkpoints, num_checkpoints, num_images = analysis_setup(test_dataset_path, input_yaml)
    full_dict_dfs, classification_reports_all_checkpoints_pixels = {}, {}

    for model_checkpoint in model_checkpoints:
        print(f'Analyzing checkpoint: {model_checkpoint}')
        checkpoint_results = {}

        for threshold in true_and_pred_matching_thresholds:
            # Initialize per-threshold storage
            data_dict_per_image = {}
            images_done = 0
            ...
            for filename in validation_image_filenames:
                ...
                predictor = get_predictor(cfg, model_checkpoint, iou_score_threshold_test, test_dataset_path)

                # Extract true and predicted data
                true_data = get_true_data_stats(cfg, defect_metadata, anno_dict_list_val, filename, model_checkpoint,
                                                threshold, iou_score_threshold_test, input_yaml['mask_on'])
                pred_data = get_pred_data_stats(cfg, defect_metadata, anno_dict_list_val, filename, predictor,
                                                model_checkpoint, threshold, iou_score_threshold_test,
                                                input_yaml['mask_on'])

                if not pred_data or not true_data:
                    continue  # Skip problematic images

                num_found, stats = match_defects(true_data, pred_data, method=true_and_pred_matching_method)
                data_dict_per_image[filename] = stats

                images_done += 1

            # Save per-image stats and calculate overall stats
            prefix = f"Threshold_{threshold}_Checkpoint_{model_checkpoint}"
            save_per_image_stats(data_dict_per_image, cfg.OUTPUT_DIR, prefix)
            calculate_stats(data_dict_per_image, cfg.OUTPUT_DIR, prefix)

            # Save plots
            for metric, (true_vals, pred_vals, color, label) in {
                "sizes": ([stats["true_sizes"] for stats in data_dict_per_image.values()],
                          [stats["pred_sizes"] for stats in data_dict_per_image.values()],
                          "blue", "Sizes"),
                "densities": ([stats["true_densities"] for stats in data_dict_per_image.values()],
                              [stats["pred_densities"] for stats in data_dict_per_image.values()],
                              "red", "Densities")
            }.items():
                plot_and_save_parity_plots(true_vals, pred_vals,
                                           x_label=f"True {label}", y_label=f"Predicted {label}",
                                           output_path=os.path.join(cfg.OUTPUT_DIR, f"{prefix}_{metric}.png"),
                                           color=color, label=label)

            checkpoint_results[threshold] = data_dict_per_image

        full_dict_dfs[model_checkpoint] = checkpoint_results
